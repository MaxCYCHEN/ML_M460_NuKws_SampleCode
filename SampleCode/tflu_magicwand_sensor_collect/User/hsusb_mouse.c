...
// Prototype
//
static void hsusb_hid_class_request(void);
static void hsusb_endpoint_setup_for_hs(void);
static void hsusb_endpoint_setup_for_fs(void);
...
{
    HSUSBD_Open(&gsHSInfo, hsusb_hid_class_request, NULL);
...
static void hsusb_endpoint_setup_for_hs(void)
{
    ...
    HSUSBD_ConfigEp(EPA, HS_INT_IN_EP_NUM, HSUSBD_EP_CFG_TYPE_INT, HSUSBD_EP_CFG_DIR_IN);
}

//
// hsusb_endpoint_setup_for_fs
//
static void hsusb_endpoint_setup_for_fs(void)
{
    /* EPA ==> Interrupt IN endpoint, address 1 */
