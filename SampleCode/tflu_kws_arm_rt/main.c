 ...
 * @file     main.c
 * @version  V3.00
 * @brief    This is an keyword spotting inference realtime V1. with TFLITE 
...
int32_t main(void)
{
    ...
    printf("KWS simple example; build timestamp: %s:%s\n", __DATE__, __TIME__);
		...
		int recordingWin = 1;  // every time update 1 frame, 1/25(s)
    ...
    //AUDIO_BUFFER_IN = audio_io_buffer1;
        ...
        if(s_u8CopyData)
        {
              ...
              kws.AveragePredictions();
                ...
                //  (static_cast<int>(kws.averagedOutput[maxIndex_av]*100)));
								...
								if(std::find(outputClass_ans.begin(), outputClass_ans.end(), outputClass[maxIndex_av]) != outputClass_ans.end()){  // skip unknown and silence 
							      printf("Trigger %s\r\n", outputClass[maxIndex_av]);	
					  ...
					  //printf("Ch2[0]\n");  
  					//printf("RxBuff0_st,RxBuff0_ed : %u, %u\n", s_au32PcmRxBuff[0][0],s_au32PcmRxBuff[0][BUFF_LEN - 1]);
					  //printf("Ch2[1]\n");  
  					//printf("RxBuff1_st,RxBuff1_ed : %u, %u\n", s_au32PcmRxBuff[1][0],s_au32PcmRxBuff[1][BUFF_LEN - 1]);
        }
    }
}
