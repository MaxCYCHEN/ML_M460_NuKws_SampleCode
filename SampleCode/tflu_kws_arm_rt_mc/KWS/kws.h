...
public:
    //KWS(int recordingWin, int slidingWindowLen, int16_t * audio_buffer);
    KWS(int slidingWindowLen, uint32_t * audio_buffer); 
    //KWS(int16_t * ptrAudioBuffer, uint32_t nElements);

    ~KWS() = default;

    void ExtractFeatures();
    void ExtractFeatures_overlap(uint8_t count);
    void Classify();
    ...
    int audioBufferSize;
		...
		int16_t audio_buffer_16bit[16000];
